...
export function InsightAuthor({ author }: InsightAuthorProps) {
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-[1320px] border border-black/10 bg-white dark:border-white/10 dark:bg-[#0f0f0f]">
        <div className="grid overflow-hidden lg:grid-cols-[minmax(0,1.05fr)_minmax(320px,429px)]">
          <div className="flex items-center px-6 py-8 md:px-10 md:py-12 lg:px-12">
            <div className="w-full max-w-xl">
              {author?.name ? (
                <p className="font-funnel text-[28px] leading-[1.15] tracking-[-1px] text-foreground md:text-[32px]">
                  {author.name}
                </p>
              ) : null}

              {author?.role ? (
                <p className="mt-2 font-funnel text-[15px] text-foreground/65 md:text-[16px]">
                  {author.role}
                </p>
              ) : null}
              ...
              {author?.bio ? (
                <p className="mt-6 max-w-[34rem] font-funnel text-[15px] leading-7 text-foreground/80 md:text-[17px] md:leading-8">
                  {author.bio}
                </p>
              ) : null}
            </div>
          ...
          <div className="border-t border-black/10 dark:border-white/10 lg:border-l lg:border-t-0">
            <div className="relative aspect-[429/523] min-h-[320px] overflow-hidden bg-[#f3f3f5] dark:bg-[#090909]">
              <div
                aria-hidden="true"
                className="pointer-events-none absolute inset-0 overflow-hidden whitespace-pre-wrap break-all px-4 pt-3 font-mono text-[32px] uppercase leading-[1.08] tracking-[0.18em] text-black/10 dark:text-white/10"
              >
                {BINARY_PATTERN}
              </div>

              {portraitUrl ? (
                <div className="absolute bottom-0 right-0 h-[82%] w-[78%] overflow-hidden bg-white/70 dark:bg-black/20 relative">
                  <Image
                    src={portraitUrl}
...
function AuthorBadge() {
  return (
    <div className="relative flex h-14 w-14 items-center justify-center border border-brand/30 bg-white/70 backdrop-blur-sm dark:bg-black/40">
      <div className="absolute left-2 top-2 h-4 w-4 rounded-[2px] border-2 border-brand" />
      <div className="absolute right-2 bottom-2 h-4 w-4 rounded-[2px] border-2 border-brand" />
